...
export default function SupplierPurchaseTotal() {
  ...
  const fetchPurchaseTotals = async () => {
    try {
      setLoading(true);
      ...
      if (payload.status === 'success') {
        ...
        alert(payload.message || 'Failed to load purchase totals');
      }
    } catch (error) {
      console.error('Failed to fetch purchase totals:', error);
      alert('Failed to load purchase totals');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="profile-content">
        <div className="profile-section">
          <p style={{ color: '#64748b', fontSize: '0.92rem', marginTop: 0, marginBottom: '1rem' }}>
            Track your upstream supplier purchase totals &mdash; what you have purchased from each supplier.
          </p>
          <div className="supplier-summary-grid">
            <div className="supplier-summary-card">
              <p className="supplier-summary-label">Upstream suppliers</p>
            ...
            <div className="supplier-summary-card">
              ...
              <p className="supplier-summary-value">
                ₹{Number(data.summary?.totalPurchaseValue || 0).toLocaleString()}
              </p>
            </div>
            <div className="supplier-summary-card">
              <p className="supplier-summary-label">Paid purchase value</p>
              <p className="supplier-summary-value">
                ₹{Number(data.summary?.paidPurchaseValue || 0).toLocaleString()}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="profile-section">
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.8rem', flexWrap: 'wrap' }}>
            <h2 style={{ margin: 0 }}>Supplier-wise details</h2>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1000px' }}>
                <thead>
                  <tr>
                    ...
                    <th style={th}>Paid Purchase Value</th>
