...
function mergeSupplierProductAfterUpdate(existingRow, apiProduct, productData = {}) {
  ...
  const offerSpecs =
    savedOfferSpecs && typeof savedOfferSpecs === 'object' && !Array.isArray(savedOfferSpecs)
      ? specificationsWithMeaningfulValuesOnly(savedOfferSpecs)
      : {};
  const isPendingOffer = String(normalized.status || 'pending').toLowerCase() === 'pending';
  const mergedDisplay = isPendingOffer
    ? offerSpecs
    : mergeCatalogAndOfferSpecificationsForDisplay(
...
const ProductModal = ({
  ...
  useEffect(() => {
    if (product) {
      const productSpecs = resolveSavedSpecificationsForSupplierEdit(product);
  ...
  }, [product?.supplierOfferSpecifications, product?.attributes?.specifications, product?.catalogSpecifications, product?.specifications]);
  ...
  useEffect(() => {
    ...
    // New products only — editing an existing listing must keep its saved spec keys.
    if (product) return;
  ...
  // Pending edit: load category template keys for validation without injecting empty spec rows.
  useEffect(() => {
    if (!product) return;
    if (String(product.status || 'pending').toLowerCase() !== 'pending') return;
    const category = String(formData.category || product.category || '').trim();
    if (!category) return;

    loadCategorySpecifications(category, String(formData.name || product.name || '').trim(), {
      preserveExistingValues: true,
      brand: formData.brand || product.brand
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [product?.id, product?.status, formData.category]);
      ...
      if (resp.ok) {
        ...
        if (data.status === 'success') {
          ...
          if (specKeys.length > 0) {
            ...
            // Editing an existing product: keep saved specs; only load template keys for validation.
            if (product) {
              selectedSuggestionSpecsRef.current = null;
              return;
            }
